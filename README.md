...
- **Redstone (`/ferrite redstone ac on`).** Adapts [Space Walker's Alternate Current](https://github.com/SpaceWalkerRS/alternate-current) algorithm into Ferrite. About 10x fewer wire cascades, and contraptions run roughly 6x faster at equivalent server load. Bit-for-bit correct against vanilla on 150,000+ oracle checks. Works on existing worlds without toggles or migration. As of 0.4.0-alpha, each cascade's power propagation also runs through a Rust kernel by default (one batched JNI call per cascade) for another ~30% wire-cost reduction on heavy contraptions; disable with `/ferrite redstone bfs off` if needed.
...
The native library is bundled for Windows, Linux, and macOS. If it fails to load on your platform, Ferrite falls back to vanilla behavior automatically, with no crashes and no broken worlds. ARM Linux is not bundled yet.

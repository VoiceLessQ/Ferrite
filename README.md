...
- **Cramming (active by default, toggle with `/ferrite cramming on|off|status`)** — a Rust reimplementation of the mob-vs-mob cramming loop. Cuts the server's entity-tick cost by roughly 65% at high mob density. Lets you keep stable TPS standing next to a 1000+ mob farm. **Trade-off:** Ferrite's cramming path does not apply vanilla cramming damage — combine with `/gamerule maxEntityCramming 0` to run unbounded farms without TPS collapse, or use `/ferrite cramming off` to fall back to vanilla (damage and slowness both restored) so you can A/B the perf claim in your own world.

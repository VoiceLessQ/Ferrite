...
> Extreme-redstone users running large sustained contraptions can enable `/ferrite redstone bfs-min 1` for additional wire performance (~30% on the lag-machine workload above) by routing AC's per-cascade propagation through a Rust kernel. Workload-shape dependent — wins on sustained high-volume cascades, can lose on bursty small ones, which is why it's off by default. See [docs/REDSTONE_PORT_PLAN.md](docs/REDSTONE_PORT_PLAN.md) for the per-bucket measurements.

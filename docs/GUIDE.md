...
Full reference for Ferrite. The [README](../README.md) has the headline numbers and quick install; this doc has measured results, full command reference, how each system works, what's still in progress, how to help, and platform notes.
...
### Sign + furnace ticker hygiene

Vanilla registers a `BlockEntityTicker` for every sign and every furnace at chunk load and ticks all of them every server tick, even though the body does nothing useful 99%+ of the time (no one is editing the sign, the furnace is empty). Ferrite suppresses the ticker for vanilla `SignBlockEntity` / `HangingSignBlockEntity` with no active editor (re-registers the moment a player opens the edit screen) and for vanilla `FurnaceBlockEntity` / `BlastFurnaceBlockEntity` / `SmokerBlockEntity` when empty, not burning, and no recipe in progress (re-registers on `setStack`, so a hopper insert wakes the furnace immediately). Strict-class check preserves mod subclass behavior — modded sign or furnace types with non-trivial tick bodies keep their tickers untouched. Self-heals from persisted-editor state within 2 ticks of chunk load. See [JOURNEY.md](JOURNEY.md) "May 2026 audit pass" for the underlying flat-data-structure pattern this exploits.
...
### Sign + furnace ticker (Hermitcraft-scale builds)

| condition             | BE-tick avg   | notes                              |
| --------------------- | ------------- | ---------------------------------- |
| 0 signs (baseline)    | ~0.08 ms/tick | empty area, JIT warm               |
| 961 signs, no fix     | ~0.20 ms/tick | every sign ticks, 99%+ no-op body  |
| 961 signs, fix active | ~0.06 ms/tick | **~70% reduction**                  |
| 500 idle furnaces     | ~0.05 ms/tick | **zero measurable add over baseline** |

Active editor on a sign or active smelting on a furnace re-registers the ticker immediately. Persisted-editor state (e.g. crash mid-edit, world reload) self-heals on the first tick after chunk load. Strict-class check preserves mod subclass tick behavior.

Measurement details in [CHANGELOG.md](../CHANGELOG.md), the full investigation path in [PROFILING.md](PROFILING.md), and a cross-port retrospective in [JOURNEY.md](JOURNEY.md).
...
- **Hopper extract hint** — skip-empty-prefix on extract loops, default-on. ~23-110 µs/call savings on partially-drained inventories.
- **Sign + furnace ticker suppression** — idle signs and idle furnaces no longer tick every server tick. Default-on, strict-class for mod compat.
